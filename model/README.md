...
**What re-running actually proves depends on `manifest.json`'s `data_source`,
`device`, and `deterministic_mode` fields.** If `data_source` is `distilled`,
none of this applies: `distil()` in `data.py` samples a live teacher endpoint
at `temperature: 0.9` over the network with no seed, so re-running `reproduce`
trains on a *different* corpus every time — neither the score nor the hash is
expected to reproduce, independent of `--deterministic` or the GPU. For
`collected`/`seed` sources (the training data itself is fixed), on the
`--tiny` CPU path `reproduce` matches the exact `sha256`. On a real
(non-`--tiny`) GPU run, backward-pass kernels are not bit-deterministic by
default — re-running will match the released `score` but is not guaranteed to
match the released `sha256` bit-for-bit. Pass `--deterministic` to opt into
`torch.use_deterministic_algorithms` plus fixed cuDNN/cuBLAS settings for a
best-effort bit-reproducible run (recorded as `"deterministic_mode":
"best_effort_warn_only"` in the manifest — "best-effort" because
`use_deterministic_algorithms` runs with `warn_only=True`: an op with no
deterministic kernel logs a warning and falls back rather than crashing
training, so this is a request for determinism, not a guarantee of it); it
costs throughput, and because the base models here aren't audited for full
deterministic-kernel coverage, it is **implemented but not yet verified
end-to-end on a real GPU** — this repo has only confirmed it on the CPU
`--tiny` path. Verifying it against a real training run, and tightening the
guarantee for any op it can't cover deterministically, is open work — see the
