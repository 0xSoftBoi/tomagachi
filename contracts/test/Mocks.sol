...
/// @notice A whitelisted vault that turns hostile: identical to MockVault4626
/// (deposits and pricing behave normally) until `setBroken(true)`, after which
/// `withdraw` always reverts — simulating a vault that bricks on withdrawal
/// (paused, insolvent, upgraded to a hostile implementation, etc). Used to
/// check that one bad vault degrades gracefully rather than bricking the
/// whole treasury.
contract MaliciousVault4626 {
    MockUSDC public immutable usdc;
    uint256 public totalSupply;
    mapping(address => uint256) public balanceOf;
    bool public broken;
